...
async function downloadDirectoryListImpl(path: string) {
  ...
  if (!Array.isArray(data)) {
    throw new Error(`GitHub should always return an array, not sure what happened for the path ${path}`);
  }
...
async function downloadFirstMdxFileImpl(list: Array<{ name: string; sha: string; type: string }>) {
...
async function downloadDirectoryImpl(path: string): Promise<Array<GitHubFile>> {
  ...
  for (const fileOrDirectory of fileOrDirectoryList) {
    switch (fileOrDirectory.type) {
      ...
      default:
        throw new Error(`Unknown file type returned for the file ${fileOrDirectory.path}`);
    }
  }
...
export async function downloadMdxOrDirectory(relativePath: string) {
  ...
  const potentials = directoryList.filter(({ name }) => name.startsWith(basename));
  const potentialDirectory = potentials.find(({ type }) => type === "dir");
  const exactMatch = potentials.find(({ name }) => nodepath.parse(name).name === nameWithoutExt);

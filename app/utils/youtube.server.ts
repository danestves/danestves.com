...
const getYouTubeAPIURL = (source: string): string => {
  return `${GOOGLE_API}${source}&key=${API_KEY}`;
};
...
export async function getVideos(props?: GetVideosProps): Promise<Videos> {
  ...
  const latestVideos = await fetch(
    getYouTubeAPIURL(`/playlistItems?part=contentDetails&playlistId=${PLAYLIST_ID}&maxResults=${limit}`)
  ...
  const result: Videos = await fetch(getYouTubeAPIURL(`/videos?part=snippet,statistics&id=${ids.join(",")}`)).then(

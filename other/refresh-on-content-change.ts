...
chokidar.watch(watchPath).on("change", (changePath) => {
  const relativeChangePath = changePath.replace(`${path.resolve(process.cwd(), "content")}/`, "");
  ...
  postJSON({
    ...
    overrideOptions: {
      hostname: "localhost",
      port: 3000,
      path: `/_content/update-content?${new URLSearchParams([["_data", "routes/_content/update-content"]])}`,
    },
  })
    .then(() => {

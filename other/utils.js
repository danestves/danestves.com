...
async function postJSON({ http = require("https"), postData, overrideOptions = {}, overrideHeaders = {} }) {
  return new Promise((resolve, reject) => {
    const postDataString = JSON.stringify(postData);
    const searchParams = new URLSearchParams([["_data", "routes/_content/refresh-content"]]);
    const options = {
...
function getChangedFiles(sha, compareSha) {
  try {
    ...
    const diff = execSync(`git diff --name-status ${sha} ${compareSha}`).toString();
